...
const DotGrid = ({
    ...
    useEffect(() => {
        const onMove = (e: MouseEvent) => {
            ...
            for (const dot of dotsRef.current) {
                ...
                if (speed > speedTrigger && dist < proximity && !dot._animating) {
                    ...
                    // Simulate inertia: push dot then spring back
                    const pushX = (dot.cx - pr.x) * 0.3 + vx * 0.02;
                    const pushY = (dot.cy - pr.y) * 0.3 + vy * 0.02;
                    const pushDuration = Math.min(0.2, resistance / 5000);
        ...
        // Only animate dots on pointer/mouse devices — touch scroll events cause phantom triggers on mobile
